...
/**
 * AMQP frame for dtx.set-timeout-ok
 * Parameter Summary:
 *      1. xa-result (short) - Confirm timeout set
 */
public class DtxSetTimeoutOk extends MethodFrame {
    ...
    public void handle(ChannelHandlerContext ctx, AmqpConnectionHandler connectionHandler) {
        // Server does not handle dtx set-timeout ok
    }

    public static AmqMethodBodyFactory getFactory() {
        return (buf, channel, size) -> {
            int xaResult = buf.readUnsignedShort();
            return new DtxSetTimeoutOk(channel, xaResult);
        };
    }
}
