...
/**
 * Check for expired messages of a given queue and remove
 */
public class ExpiredMessageRemoveTask extends Task {

    private static final Logger LOGGER = LoggerFactory.getLogger(ExpiredMessageRemoveTask.class);
     ...
     */
    public ExpiredMessageRemoveTask(QueueHandler queueHandler, int expiryCheckBatchSize) {
    ...
    public TaskHint call() throws Exception {
        ...
        for (Message expiredMessage : expiredMessages) {
            queueHandler.dequeue(expiredMessage.getDetachableMessage());
            LOGGER.warn("Removed expired message id = {} from queue {}. Expiry time = {}",
