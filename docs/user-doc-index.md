# User Documentation index

- [Configuring Message Broker](user/configure.md)
  - [Changing the Default MB Database](user/database-setup-guide.md)
  - [Configuring High Availability for the Broker](user/configuring-high-availability.md)
  - [Changing the Default Authentication Settings](user/authentication-settings-guide.md)
  - [Changing the Default Authorization Settings](user/authorization-settings-guide.md)
  - [Configuring Secure Vault for the Broker](user/configuring-secure-vault.md)
- [Messaging Metrics](user/messaging-metrics.md)
- [Using JMS selectors](user/jms-selector-guide.md)
- [Local Transaction](user/local-transaction.md)
- [Broker Admin Command Line Interface (CLI)](user/cli-client.md)
- [Embed Message Broker](user/embed-message-broker.md)
